...
--exec SALCAL_MAIN 4,2025,3,0,'-1',0
...
set @ROUND_SALARY_UNIT = (SELECT [Value] FROM tblParameter WHERE Code = 'ROUND_SALARY_UNIT')
 ...
 set @ROUND_TOTAL_WORKINGDAYS = ISNULL(@ROUND_TOTAL_WORKINGDAYS,50)
...
select EmployeeID,isSalCal,isLowTaxableInCome,PayrollCutOffDate
  into #tmpMonthlyPayrollCheckList from tblMonthlyPayrollCheckList where Month = @Month and Year= @Year

	if @PeriodID = 1
	delete #tmpMonthlyPayrollCheckList where ISNULL(PayrollCutOffDate,DATEADD(day,1,@todate))  not between @FromDate and @ToDate
	
 select te.EmployeeID,te.DivisionID,te.DepartmentID,te.SectionID,te.GroupID,te.EmployeeTypeID,te.PositionID,te.EmployeeStatusID,te.Sex,te.Birthday
 ,case when te.HireDate > @fromDate then cast(1 as bit) else 0 end as NewStaff
 ,CAST(0 as bit) as TerminatedStaff
 ,HireDate,CAST(NULL as datetime) TerminateDate,ProbationEndDate,te.LastWorkingDate,te.contractcode,te.ContractStartDay
 ...
 --Hieu: ngày kết thúc thử việc
 --select * from #tblEmployeeIDList
 --return
	select * into #fn_EmployeeStatus_ByDate from dbo.fn_EmployeeStatus_ByDate(@ToDate)
	select * into #fn_EmployeeStatus_ByDate_FirstLastMonth from dbo.fn_EmployeeStatus_ByDate(dateadd(dd,1,@ToDate))
	
	--lay trang thai ben bang history cho chinh xac
	UPDATE #tblEmployeeIDList SET EmployeeStatusID = stt.EmployeeStatusID
	from #tblEmployeeIDList te inner join #fn_EmployeeStatus_ByDate stt on te.EmployeeID = stt.EmployeeID
--Hieu: nghỉ việc
	UPDATE #tblEmployeeIDList SET EmployeeStatusID = stt.EmployeeStatusID
	, TerminateDate = stt.ChangedDate
	,LastWorkingDate = dateadd(dd,-1,stt.ChangedDate)
	from #tblEmployeeIDList te inner join #fn_EmployeeStatus_ByDate_FirstLastMonth stt on te.EmployeeID = stt.EmployeeID
	where stt.EmployeeStatusID = 20

	UPDATE #tblEmployeeIDList SET TerminatedStaff = 1 where TerminateDate is not null
	select * into #tblEmployeeIDList_Needdelete from #tblEmployeeIDList where 1=0
	-- ko nằm trong danh sách thì ko tính lương nha
	insert into #tblEmployeeIDList_Needdelete
	select * from (	delete #tblEmployeeIDList output deleted.* from #tblEmployeeIDList  te
	where  te.EmployeeID  not in (select EmployeeID from #tmpMonthlyPayrollCheckList where isSalCal = 1)) de

	if @@ROWCOUNT >0
	begin
		-- delete các bảng thực
		
		select EmployeeID,@Month as Month,@year as Year,@PeriodID as PeriodID into #delete_data from #tblEmployeeIDList_Needdelete

		delete #delete_data from #delete_data de
		...
		delete '+PhysictableName+' from '+physictablename+' p inner join #delete_data d on p.employeeId = d.EmployeeID and p.Month = d.Month and p.Year = d.Year and p.PeriodID = d.PeriodID' from #NameOfPhysicTables
	...
	--Junzu - Hieu: Xoa di de test luong cho nhanh
	---- những ai chưa khóa công thì ko cho tính lương
	insert into tblProcessErrorMessage(ErrorType,ErrorDetail,LoginID)
	select N'Khóa công!','['+de.EmployeeID+N']	-- Bạn chưa khóa công!',@LoginID from
	(delete #tblEmployeeIDList output deleted.EmployeeID from #tblEmployeeIDList e
	where not exists(select 1 from tblAtt_LockMonth f where e.EmployeeID = f.EmployeeID and f.Month= @Month and f.Year = @Year)
	)de
	
	delete c from #tmpMonthlyPayrollCheckList c
	where employeeID not in(select employeeID from #tblEmployeeIDList)
...
-- lấy dữ liệu bảng custom attendance ra
 ...
 and ot.EmployeeID in(select te.EmployeeID from #tblEmployeeIDList te
	...
	delete #tblOTList from #tblOTList ot where not exists(select 1 from #tblEmployeeIDList te where ot.EmployeeID = te.EmployeeID and ot.OTDate between te.HireDate and te.LastWorkingDate)
	...
	SET @MaxSalaryHistoryId = (SELECT MAX(SalaryHistoryID) FROM #tblSalDetail)+7121997 --TN:fake id cao lên để tránh trùng lắp với ID real
		...
		select sh.EmployeeID,SalaryHistoryID, SalaryHistoryID + @MaxSalaryHistoryId ,case when @FromDate < te.HireDate then te.HireDate else @FromDate end,te.ProbationEndDate,sh.BasicSalary,sh.BasicSalary*@PROBATION_PERECNT/100.0,sh.SalCalRuleID,1,sh.BaseSalRegionalID,IsNet,PayrollTypeCode,sh.WorkingHoursPerDay,sh.CurrencyCode
		from #tblSalDetail sh inner join #tblEmployeeIDList te on sh.EmployeeID = te.EmployeeID
		where @PROBATION_PERECNT > 0 and te.ProbationEndDate >= @FromDate and ProbationEndDate < @ToDateTruncate and te.HireDate <> te.ProbationEndDate
	end

	--select ToDate,@ToDate,* from #tblSalDetail
	...
	--Hieu: Ngày hiệu lực lương

	--dong luong moi, nhung đã nghỉ việc
	select sal.EmployeeID, sal.SalaryHistoryID into #NewSalaryButTerminated from #tblSalDetail sal
	...
	delete sal from #tblSalDetail sal
	where sal.LatestSalEntry = 1 and exists(select 1 from #NewSalaryButTerminated n where sal.EmployeeID = n.EmployeeID)
	update sal set ProbationSalaryHistoryID = null, LatestSalEntry = 1 from #tblSalDetail sal
	where sal.LatestSalEntry = 0 and exists(select 1 from #NewSalaryButTerminated n where sal.EmployeeID = n.EmployeeID)
	
	update #tblSalDetail set isTwoSalLevel = 0 from #tblSalDetail sal
	where exists(select 1 from #NewSalaryButTerminated n where sal.EmployeeID = n.EmployeeID)
...
exec SALCAL_CUSTOMIZE_TADATA @StopUPDATE output, @Month,@Year, @FromDate ,@ToDate ,@LoginID,@PeriodID,@EmployeeID
	...
	END
		...
		select
		EmployeeID,ta.SalaryHistoryID,SUM(case when ISNULL(HolidayStatus,0)<>1 then 1 else 0 end )as STD_PerHistoryID
	...
	--UPDATE tblParameter SET Value = 'OTH' where Code = 'NO_TA_LEAVE_CODE'
	...
	select   LeaveCode,EmployeeID,round(SUM(LvAmount),@ROUND_ATTDAYS) as LvAmount
	into #LeaveTMpDAta from (
	...
	exec(@altertblSal_AttDataSumary_ForReport)
		...
		from #tblEmployeeIDList e where not exists(select 1 from tblSal_AttDataSumary_ForReport
		 s where e.EmployeeID = s.EmployeeID and s.Month= @Month and s.Year = @Year and s.PeriodID= @PeriodID)
	...
	cross apply dbo.fn_CurrentSalaryByDate(@FromDate, @LoginID) c where sal.SalaryHistoryID = c.SalaryHistoryID
...
------------------------------Calculate salary per day------------------------------------------
	...
	--Hieu: tính lại lương tăng ca bằng cách lấy cuối tháng
	--update #tblSalDetail set OTSalary = u.OTSalary from #tblSalDetail as s
	--	inner join ( select EmployeeID,OTSalary from  dbo.fn_CurrentOTSalary_byDate(@ToDate,@LoginID) fnTO) as u on u.EmployeeID = s.EmployeeID



	if @PROBATION_PERECNT > 0 and @PROBATION_PERECNT < 100.0
	UPDATE #tblSalDetail SET OTSalary = OTSalary*@PROBATION_PERECNT/100.0
	...
	UPDATE #tblSalDetail SET SalaryPerDay =
	 ...
	 ,SalaryPerDayOT =
	 case when isnull(IsDaily,0)= 1 then BasicSalary
		 when isnull(IsHourly,0)= 0 then OTSalary/STD_WD end
	from #tblSalDetail sal
	inner join tblSalaryCalculationRule sc on sal.SalCalRuleID= sc.SalCalRuleID
...
--Hiếu: cập nhật tiền nghỉ hết hàng
DECLARE @CongChuanCuaThang int = (select top 1 STD_WD from #tblSalDetail )
DECLARE @LuongVung1 money = (select top 1 Salary from tblBaseSalRegional where BaseSalRegionalID = 1 order by EffectiveDate desc)
UPDATE a SET AmountPaid =
case when a.LeaveCode = 'WFI' then (isnull(@LuongVung1,0) / isnull(@CongChuanCuaThang,0))
else (PaidRate/100.00) * LeaveDays* SalaryPerDay end,
SalaryPerDay = case when a.LeaveCode = 'WFI' then (isnull(@LuongVung1,0) / isnull(@CongChuanCuaThang,0)) end
,AmountDeduct =
case when a.LeaveCode = 'WFI' then 0
else
(UnpaidRate/100.00) * LeaveDays* SalaryPerDay end-- ContractSalary * (UnpaidRate/100.00) * (LeaveDays/TotalDays) --chinh xac hon lay SalaryPerDays * LeaveDays
from #TableVarLeaveAmount a
...
--Hiếu-JUNZ: cập nhật lại số ngày làm việc mặc định cho người nước ngoài là 26 dù tháng đó làm 27 ngày
DECLARE @DaysOfSalEntry float = (select value from tblParameter where Code = 'DaysOfSalEntry_NNN' )
update sal set sal.DaysOfSalEntry = sal.STD_WD
from #tblSalDetail sal
inner join #tblEmployeeIDList list on list.EmployeeID = sal.EmployeeID
where DepartmentID = 12 and sal.DaysOfSalEntry >  26
...
--khong round nua, khi tong cong dua vao bang Sal_Sal moi round
...
update al set SalaryHistoryID = ISNULL(s.ProbationSalaryHistoryID,s.SalaryHistoryID) from #tblAllowance al
...
--	,Raw_ExchangeRate float(53)
exec sp_AllowanceChildList @LoginID=@LoginID,@Month=@Month,@Year=@Year,@NoSelect=1

--Hieu: insert dự liệu con nhỏ vào Allowance
select al.EmployeeID, acpm.Month, acpm.Year, sum(al.AL_Child) DefaultMoney
into #tblAllowanceChildList
from tblAllowanceChildList al
inner join tblAllowanceChildPerMonth acpm on acpm.ChildID = al.ChildID
where al.EmployeeID in (select EmployeeID from #tblEmployeeIDList)
and acpm.Month = @Month and acpm.Year = @Year and ISNULL(acpm.ApprovedC,0) = 1
group by  al.EmployeeID,acpm.Month,acpm.Year,al.AL_Child;

--Cập nhật số tiền trợ cấp con nhỏ cho đúng với bên bảng danh sách trợ cấp con nhỏ
if @Year*12+@Month >= 2024*12+6
begin
	--dua ve 0 het
	UPDATE #tblAllowance SET DefaultAmount = 0 where AllowanceID = 11

	--lay tien ben kia qua
	update al set al.DefaultAmount = p.DefaultMoney
	from #tblAllowance al
	inner join #tblAllowanceChildList p on p.EmployeeID = al.EmployeeID
	inner join #tblSalDetail sal on p.EmployeeID = sal.EmployeeID and sal.LatestSalEntry = 1 and sal.DaysOfSalEntry > 0
	where al.AllowanceID = 11 and al.DefaultAmount = 0
end
...
if @PROBATION_PERECNT < 100.0
	begin
		...
		update  #tblAllowance set DefaultAmount = DefaultAmount*@PROBATION_PERECNT/100.0
		from #tblAllowance al
		inner join #tblEmployeeIDList te on al.EmployeeID = te.EmployeeID
		where te.ProbationEndDate >= @ToDateTruncate and @PROBATION_PERECNT > 0 and al.DefaultAmount > 0
		and te.HireDate <> te.ProbationEndDate
		...
		where (al.LatestSalEntry = 0 or (al.LatestSalEntry = 1 and n.EmployeeID is not null))and al.DefaultAmount > 0 and  @PROBATION_PERECNT > 0 and te.ProbationEndDate >= @FromDate and te.ProbationEndDate < @ToDateTruncate
		and te.HireDate <> te.ProbationEndDate
	end
...
DECLARE @HEALTH_CHECKUP_PRICE money, @BIRTHDAY_BONUS money
select @HEALTH_CHECKUP_PRICE = Value from tblParameter where Code = 'HEALTH_CHECKUP_PRICE'
SET @HEALTH_CHECKUP_PRICE = ISNULL(@HEALTH_CHECKUP_PRICE,200000)
select @BIRTHDAY_BONUS = Value from tblParameter where Code = 'BIRTHDAY_BONUS'
SET @BIRTHDAY_BONUS = ISNULL(@BIRTHDAY_BONUS,150000)
DECLARE @BREAKUP_BONUS_HOURS_OT float
select @BREAKUP_BONUS_HOURS_OT = Value from tblParameter where Code = 'BREAKUP_BONUS_HOURS_OT'


--Hieu: Adj_HealthCheckup - thêm danh sách nhân viên mới nhận phụ cấp khám sức khỏe
INSERT INTO #tblAdjustment(EmployeeID,IncomeID,ByAmount,SalaryPercent,AdjustmentAmount,CurrencyCode,ExchangeRate)
select list.EmployeeID,29 IncomeID,1,0,@HEALTH_CHECKUP_PRICE,'VND',0
from #tblEmployeeIDList list
where HireDate between @FromDate and @ToDate --and EmployeeStatusID <> 20
AND NOT EXISTS (SELECT 1 FROM #tblAdjustment a WHERE a.EmployeeID = list.EmployeeID AND a.IncomeID = 29)
and list.DepartmentID <> 12
--Hieu: kiểm tra xem nhân viên đó có nghỉ việc hay sắp nghỉ việc chưa, nếu sắp thì sẽ thêm phụ cấp bằng cách kiểm tra số ngày công có >50% không


--Hieu: phu cap tham nien theo hop dong
--Hieu: Adj_HealthCheckup - thêm danh sách nhân viên mới nhận phụ cấp khám sức khỏe
INSERT INTO #tblAdjustment(EmployeeID,IncomeID,ByAmount,SalaryPercent,AdjustmentAmount,CurrencyCode,ExchangeRate)
SELECT list.EmployeeID,39 AS IncomeID,1 AS [Type],0 AS [IsSubtract],
    CASE
        WHEN MONTH(ct.ContractStartDay) = @month AND YEAR(ct.ContractStartDay) = @year THEN 200000
        WHEN MONTH(ct.ContractStartDay) = @month AND YEAR(ct.ContractStartDay) < @year THEN 100000
        ELSE NULL
    END ,
    'VND' ,
    0
FROM #tblEmployeeIDList list
INNER JOIN tblLabourContract ct ON ct.EmployeeID = list.EmployeeID
WHERE ct.ContractCode IN ('003','008')
    AND NOT EXISTS (SELECT 1 FROM #tblAdjustment a WHERE a.EmployeeID = list.EmployeeID AND a.IncomeID = 39)
    AND list.DepartmentID <> 12
    AND MONTH(ct.ContractStartDay) = @month
    AND (YEAR(ct.ContractStartDay) = @year OR YEAR(ct.ContractStartDay) < @year)


DECLARE @ToDateNextMonth datetime
SET @ToDateNextMonth = DATEADD(ss,-1,DATEADD(month,2,@FromDate))
select EmployeeID,SUM(DaysOfSalEntry) DaysOfSalEntry into #tmpSumDayOfSalEntry from #tblSalDetail  group by EmployeeID

----tro cap sinh nhật - Adj_BirthDay, nếu vào làm sau ngày sinh nhật thì sẽ không nhận được, chỉ cho nhân viên chính thức, nếu hết thử việc trong tháng thì phải hết trước
--ngày 13 để đủ công chuẩn
INSERT INTO #tblAdjustment(EmployeeID, IncomeID, ByAmount, SalaryPercent, AdjustmentAmount, CurrencyCode, ExchangeRate)
SELECT a.EmployeeID, 30 AS IncomeID, 1, 0, @BIRTHDAY_BONUS AS Amount, 'VND', 0
FROM #tblEmployeeIDList a
INNER JOIN #tmpSumDayOfSalEntry b ON a.EmployeeID = b.EmployeeID
INNER JOIN #tblSalDetail s ON a.EmployeeID = s.EmployeeID AND s.LatestSalEntry = 1
WHERE MONTH(Birthday) = @Month
    AND (
        a.TerminateDate IS NULL
        OR (
            a.TerminateDate BETWEEN @FromDate AND @ToDateNextMonth
            AND b.DaysOfSalEntry > s.STD_WD / 2.0
        )
    )
    AND a.DepartmentID <> 12
    AND Birthday <= HireDate
    AND a.HireDate < @ToDateNextMonth
    AND (
        a.ProbationEndDate < @FromDate
        OR (
            a.ProbationEndDate <= DATEADD(DAY, 13 - 1, @FromDate)
            AND b.DaysOfSalEntry >= s.STD_WD / 2.0
        )
    )


---tro cap phu nu - Adj_Menstrual
INSERT INTO #tblAdjustment(EmployeeID,IncomeID,ByAmount,SalaryPercent,AdjustmentAmount,CurrencyCode,ExchangeRate)
select a.EmployeeID,28 IncomeID,1,0,ROUND((ISNULL(ab.SIIncome,0) )/(a.STD_WD*8.0)*1.5*1.5,0) as Amount,'VND',0
from #tblSalDetail a inner join #tblEmployeeIDList te on a.EmployeeID = te.EmployeeID
inner join #tmpSumDayOfSalEntry total on a.EmployeeID = total.EmployeeID
left join #tblAllowance b on a.EmployeeID = b.EmployeeID and a.LatestSalEntry = b.LatestSalEntry and a.SalaryHistoryID = b.SalaryHistoryID and b.AllowanceID = 1
inner join (select EmployeeID,SIIncome from tblSal_Insurance where Month = @Month and Year = @Year) ab on te.EmployeeID = ab.EmployeeID
where a.LatestSalEntry = 1 and te.Sex = 0 --and (a.DaysOfSalEntry > a.STD_WD/2)
	and (te.TerminateDate is null
	or (te.TerminateDate between @FromDate and @ToDateNextMonth and a.DaysOfSalEntry > a.STD_WD/2.0)
)
and te.DepartmentID <>  12
and a.DaysOfSalEntry >0


--Hiếu: trợ cấp thôi việc - Adj_Adj_Unemployment
insert into #tblAdjustment(EmployeeID,IncomeID,ByAmount,AdjustmentAmount, CurrencyCode,ExchangeRate)
select sd.EmployeeID,35 InComeID, 1 as ByAmount, isnull(Severance_ALL,0), 'VND' CurrencyCode, 1 ExchangeRate
from #tblSalDetail sd
--inner join #tblEmployeeIDList te on sd.EmployeeID = te.EmployeeID
inner join tblTerminationDecision td on td.EmployeeID = sd.EmployeeID
where td.TerminatedDate between @FromDate and @ToDate and sd.LatestSalEntry = 1
and not exists (select 1 from #tblAdjustment a where a.EmployeeID = sd.EmployeeID and a.IncomeID = 12)
--and te.DepartmentID <>  12

--Hiếu: chạy lại thủ tục tính toán phép năm còn lại
exec HR_LeaveBudget_Initialization @loginID,@year,'AL','-1',@todate
exec sp_HR_ProcessTerminateData @FromDate,@ToDate,@LoginID

--Hiếu: tính tiền phép năm chưa sử dụng - Adj_Adj_AnnualLeave
insert into #tblAdjustment(EmployeeID,IncomeID,ByAmount,AdjustmentAmount, CurrencyCode,ExchangeRate)
select sd.EmployeeID,36 InComeID, 1 as ByAmount, isnull(Unused_AL,0), 'VND' CurrencyCode, 1 ExchangeRate
from #tblSalDetail sd inner join #tblEmployeeIDList te on sd.EmployeeID = te.EmployeeID
inner join tblTerminationDecision td on td.EmployeeID = sd.EmployeeID
where td.TerminatedDate between @FromDate and @ToDate
and not exists (select 1 from #tblAdjustment a where a.EmployeeID = sd.EmployeeID and a.IncomeID = 14)
and te.DepartmentID <>  12 and sd.LatestSalEntry = 1

--Hiếu: tính tiền phép năm tồn của năm cũ
SELECT
    te.EmployeeID,
    (ISNULL(prev.Remain, 0) - isnull(curr.LastYear,0)) AS LeaveBalance
INTO #tblEmployeAnuualRemaining
FROM #tblEmployeeIDList te
INNER JOIN tblEmployeeAnnual curr ON te.EmployeeID = curr.EmployeeID AND curr.CYear = @Year
LEFT JOIN tblEmployeeAnnual prev ON te.EmployeeID = prev.EmployeeID AND prev.CYear = @Year - 1;

DECLARE @AL_EXPIRE date =  (select top 1 cast(Value+'/'+cast(@Year as varchar(4)) as datetime)from tblParameter tp where tp.Code='HR_AL_EXPIRE_DATE')

--INSERT INTO #tblAdjustment(EmployeeID,IncomeID,ByAmount,SalaryPercent,AdjustmentAmount,CurrencyCode,ExchangeRate)
--select a.EmployeeID,38 IncomeID,1,0,ROUND((ab.SIIncome / (a.STD_WD)) * remain.LeaveBalance,0) as Amount,'VND',0
--from #tblSalDetail a inner join #tblEmployeeIDList te on a.EmployeeID = te.EmployeeID
--inner join (select EmployeeID,SIIncome from tblSal_Insurance where Month = @Month and Year = @Year) ab on te.EmployeeID = ab.EmployeeID
--left join #tblEmployeAnuualRemaining remain on te.EmployeeID = remain.EmployeeID
--where a.LatestSalEntry = 1
--	and (te.TerminateDate is null	
--) and @Month = month(@AL_EXPIRE)
--and te.DepartmentID <>  12


select a.EmployeeID,datediff(day,@fromdate,hiredate) as KhoangCach into #tempKhoangCach
from #tblSalDetail a inner join #tblEmployeeIDList te on a.EmployeeID = te.EmployeeID
where a.LatestSalEntry = 1
	and (te.TerminateDate is null
	or (te.TerminateDate between @FromDate and @ToDateNextMonth and a.DaysOfSalEntry > a.STD_WD/2.0)
) and te.DepartmentID not in (1,12)

--Hiếu: trợ cấp nghỉ ngắn cho nhân viên
select * into #tblDepartmentShortAL from tblDepartment where isShortAL = 1
select * into #tblSectionShortAL from tblSection where isShortAL = 1


INSERT INTO #tblAdjustment(EmployeeID,IncomeID,ByAmount,SalaryPercent,AdjustmentAmount,CurrencyCode,ExchangeRate)
select a.EmployeeID,37 IncomeID,1,0,ROUND(ab.SIIncome / (a.STD_WD * 8) *  (isnull(@BREAKUP_BONUS_HOURS_OT,5) * 1.5),0) as Amount,'VND',0
from #tblSalDetail a inner join #tblEmployeeIDList te on a.EmployeeID = te.EmployeeID
inner join (select EmployeeID,SIIncome from tblSal_Insurance where Month = @Month and Year = @Year) ab on te.EmployeeID = ab.EmployeeID
left join #tblDepartmentShortAL al on al.DepartmentID = te.DepartmentID
left join  #tblSectionShortAL sec on sec.SectionID = te.SectionID
where a.LatestSalEntry = 1
	and (te.TerminateDate is null
	or (te.TerminateDate between @FromDate and @ToDateNextMonth and a.DaysOfSalEntry > a.STD_WD/2.0)
)
and a.DaysOfSalEntry >0
and @FromDate >= '2024-09-01'
AND ((sec.isShortAL = 1) OR (sec.SectionID IS NULL AND al.isShortAL = 1));

--Hiếu: update lại tiền nghỉ ngắn của nhân viên theo lũy tiến 3 ngày - 0.5 tiền thưởng tăng ca
-- 3 => 0.5
-- 6 => 1
-- 9 => 1.5
...
update a set a.AdjustmentAmount =
 CASE
        WHEN kc.KhoangCach >= 3 THEN
            CASE
                WHEN a.AdjustmentAmount - ROUND((FLOOR(kc.KhoangCach / 3) * 0.5) * (SalaryPerHour * 1.5), 0) < 0 THEN 0
                ELSE a.AdjustmentAmount - ROUND((FLOOR(kc.KhoangCach / 3) * 0.5) * (SalaryPerHour * 1.5), 0)
            END
        ELSE a.AdjustmentAmount
    END
from #tblAdjustment a
inner join #tblSalDetail sd on a.EmployeeID = sd.EmployeeID
inner join #tempKhoangCach kc on kc.EmployeeID = a.EmployeeID
where a.IncomeID = 37

update a set a.AdjustmentAmount =
  CASE
        WHEN b.LvAmount >= 3 THEN
            CASE
                WHEN a.AdjustmentAmount - ROUND(((FLOOR(b.LvAmount / 3) * 0.5) * (ab.SI_Salary / (sd.STD_WD * 8)) * 1.5), 0) < 0 THEN 0
                ELSE a.AdjustmentAmount - ROUND(((FLOOR(b.LvAmount / 3) * 0.5) * (ab.SI_Salary / (sd.STD_WD * 8)) * 1.5), 0)
            END
        ELSE a.AdjustmentAmount
    END
from #tblAdjustment a
inner join #tblSalDetail sd on a.EmployeeID = sd.EmployeeID
inner join (
	select EmployeeID, SUM(LvAmount)/8 LvAmount from #tblLvHistory where LeaveCode not in( 'AL','PH') group by EmployeeID
	having SUM(LvAmount) > 0
) b on a.EmployeeID = b.EmployeeID
inner join (select EmployeeID, SI_Salary from fn_CurrentSISalary_byDate(@ToDate,null)) ab on a.EmployeeID = ab.EmployeeID
where a.IncomeID = 37 --and sd.LatestSalEntry = 1
select EmployeeID, SUM(LvAmount)/8 LvAmount from #tblLvHistory where LeaveCode not in( 'AL','PH') group by EmployeeID
	having SUM(LvAmount) > 0
...
exec SALCAL_ADJUSTMENT_BEFORE_INSERT @StopUPDATE output, @Month,@Year, @FromDate ,@ToDate ,@LoginID,@PeriodID,@EmployeeID
 ...
 create table #tblOT_Split_Final
 (
	EmployeeID varchar(20),
	OTKind int,
	Hours float,
 ...
 Insert into #tblOT_Split_Final
 EXEC [sp_TestOverOT] @loginID,@Month,@Year
...
--,s.SalaryPerDayOT, s.SalaryPerHourOT,s.STD_WD, s.LatestSalEntry,MAX(os.NSPercents) as NSPercents,CAST(0 as  money ) as NightShiftAmount,s.OTSalary
...
--group by o.OTKind,o.EmployeeID,s.SalaryHistoryID,os.OvValue,s.SalaryPerDayOT, s.SalaryPerHourOT,s.SalaryPerHour,s.STD_WD,s.LatestSalEntry,s.OTSalary
select
      o.OTKind
    , o.EmployeeID
    , @Month Month
    , @Year Year
    , s.SalaryHistoryID
    , os.OvValue as OTRate
    , SUM(o.Hours) as OTHour -- Lấy từ bảng Split
    , CAST(0 as float(53)) OTAmount
    , s.SalaryPerDayOT
    , s.SalaryPerHourOT
    , s.STD_WD
    , s.LatestSalEntry
    , MAX(os.NSPercents) as NSPercents
    , CAST(0 as money) as NightShiftAmount
    , s.OTSalary
    , o.IsOver200 -- <--- QUAN TRỌNG: Thêm cột đánh dấu này vào
into #tblSal_OT_Detail
from #tblOT_Split_Final o -- <--- ĐỔI TỪ #tblOTList SANG BẢNG NÀY
inner join #tblSalDetail s on o.EmployeeID = s.EmployeeID -- Bỏ điều kiện Date vì bảng Split đã tổng hợp theo tháng rồi
inner join tblOvertimeSetting os on o.OTKind = os.OTKind
group by o.OTKind, o.EmployeeID, s.SalaryHistoryID, os.OvValue, s.SalaryPerDayOT, s.SalaryPerHourOT, s.SalaryPerHour, s.STD_WD, s.LatestSalEntry, s.OTSalary, o.IsOver200
...
-- add thêm phần night shift
	UPDATE #tblSal_OT_Detail set
	 OTAmount = isnull(ROUND(o.OTRate*o.OTHour*o.SalaryPerHourOT/100.0,0),0)
	 ,NightShiftAmount = ROUND(o.NSPercents*o.OTHour*o.SalaryPerHourOT/100.0,0)
...
select
    ...
    , SalaryHistoryID
    , LatestSalEntry
    , SUM(isnull(OTAmount,0)) TotalOTAmount
    , SUM(isnull(NightShiftAmount,0)) NightShiftAmount
    , CAST(NULL as money) TaxableOTAmount
    , CAST(NULL as money) NoneTaxableOTAmount
    , IsOver200 -- Group theo cái này để tách dòng thuế
into #tmpOvertimeSummary
from #tblSal_OT_Detail
group by EmployeeID, OTKind, OTRate, SalaryHistoryID, LatestSalEntry, IsOver200

UPDATE #tmpOvertimeSummary
SET NoneTaxableOTAmount = ROUND(TotalOTAmount - (TotalOTAmount/OTRate*100), 0)
WHERE IsOver200 = 0

UPDATE #tmpOvertimeSummary
SET NoneTaxableOTAmount = 0
WHERE IsOver200 = 1
select * from #tblSal_OT_Detail
select * from #tmpOvertimeSummary return

UPDATE #tmpOvertimeSummary SET TaxableOTAmount = TotalOTAmount - NoneTaxableOTAmount


UPDATE #tblSalDetail SET TaxableOTTotal = round(tmp.TaxableOTAmount,@ROUND_SALARY_UNIT)
...
from #tblSalDetail s
cross apply (
	select sum(TaxableOTAmount) TaxableOTAmount, sum(NoneTaxableOTAmount) NoneTaxableOTAmount, sum(TotalOTAmount) TotalOTAmount, sum(NightShiftAmount) NightShiftAmount
	from #tmpOvertimeSummary tmp where s.EmployeeID = tmp.EmployeeID and s.LatestSalEntry = tmp.LatestSalEntry) tmp
...
where AllowanceRuleID IN (3,4,5,7,8,11,17)

--Hieu: update lại phụ cấp con nhỏ để không đổi công thức
select EmployeeID,SUM(DaysOfSalEntry) DaysOfSalEntry into #tmpSumDayOfSalEntryChil from #tblSalDetail  group by EmployeeID
update al set ReceiveAmount = al.DefaultAmount*total.DaysOfSalEntry/sal.STD_WD
from #tblAllowance al
inner join #tmpSumDayOfSalEntryChil total on al.EmployeeID = total.EmployeeID
inner join #tblSalDetail sal on sal.EmployeeID = al.EmployeeID
where al.AllowanceID = 11
...
WHERE ApprovedDeduct = 1 and ApprovedDeduct > 0 AND IODate BETWEEN @FromDate AND @ToDate
 AND exists(SELECT 1 FROM #tblEmployeeIDList te where il.EmployeeID = te.EmployeeID and il.IODate between te.HireDate and te.LastWorkingDate)

UPDATE #tblDiligentAll SET ActualQty = tmp.IOMinute
FROM #tblDiligentAll d
INNER JOIN (SELECT EmployeeID, SUM(IOMinutesDeduct) AS IOMinute FROM #tblIOData GROUP BY EmployeeID) tmp ON d.EmployeeID = tmp.EmployeeID
...
UPDATE #tblDiligentAll SET ActualQty = tmp.IOMinute
FROM #tblDiligentAll d
INNER JOIN (SELECT EmployeeID,count(1) AS IOMinute FROM #tblIOData GROUP BY EmployeeID) tmp ON d.EmployeeID = tmp.EmployeeID
...
FROM #tblDiligentAll D
INNER JOIN(SELECT EmployeeID,Count(1) NotOTCount FROM tblNotOtList ot
		   WHERE OTDate BETWEEN @FromDate AND @ToDate AND Approved = 1 and exists(select 1 from #tblEmployeeIDList te where ot.EmployeeID = te.EmployeeID and ot.OTDate between te.HireDate and te.LastWorkingDate)
...
FROM #tblDiligentAll D
INNER JOIN (SELECT EmployeeID,SUM(CASE WHEN LeaveStatus = 3 THEN 2 ELSE 1 END) FWCCount FROM #tblLvHistory WHERE LeaveCode = 'FWC' GROUP BY EmployeeID) TMP ON d.EmployeeID = tmp.EmployeeID
...
UPDATE #tblDiligentAll SET ActualQty = lv.lvCount
FROM #tblDiligentAll D
cross apply (select SUM(lv.LvAmount)/8.0 lvCount from #tblLvHistory lv where D.EmployeeID = lv.EmployeeID and D.Code = lv.LeaveCode) lv
...
INNER JOIN (
	...
	INNER JOIN (select SUM(lv.LvAmount)/8.0 AS lvCount,EmployeeID
		from #tblLvHistory lv where isnull(lv.CutDiligent ,0)= 1
...
select distinct AllowanceCode,AllowanceID  into #CustomColname
from tblAllowanceSetting
where AllowanceCode in (select Column_Name from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME= 'tblCustomInputImportMonthly') and Visible = 1

 select * into #tblCustomInputImportMonthly from tblCustomInputImportMonthly c
 where Month = @Month and Year = @Year
 and exists(select 1 from #tblEmployeeIDList te where c.EmployeeID = te.EmployeeID)
 ...
 begin
		select top 1 @AllowanceCustomInput = AllowanceCode
		...
		delete #CustomColname where AllowanceCode = @AllowanceCustomInput
 end
...
update #tblAllowance set TaxableAmount = 0 , UntaxableAmount = 0 where isnull(ReceiveAmount,0) = 0
...
--tringuyen
update #AdjustmentSum set TaxableAmount = 0, UntaxableAmount = 0 where isnull(AdjustmentAmount,0) = 0
...
delete #tblAllowance where isnull(DefaultAmount,0) = 0 and isnull(ReceiveAmount,0) = 0
...
insert into #tblSal_Allowance_Detail_des (
	...
	,@PeriodID
	,sum(a.TotalPaidDays)
	,MAX(cast(LatestSalEntry as int))
	...
	SELECT al.EmployeeID,al.AllowanceID,@Year,@Month,ROUND(SUM(ISNULL(al.ReceiveAmount,0)),@ROUND_SALARY_UNIT),al.TakeHome
		,ROUND(sum(UntaxableAmount),@ROUND_SALARY_UNIT) as UntaxableAmount,ROUND(sum(TaxableAmount),@ROUND_SALARY_UNIT) as TaxableAmount
		,ROUND(sum(case when LatestSalEntry=1 then Raw_DefaultAmount else 0 end),@ROUND_SALARY_UNIT) as Raw_DefaultAmount
		,max(Raw_CurrencyCode) as Raw_CurrencyCode
		,max(Raw_ExchangeRate) as Raw_ExchangeRate
		,ROUND(sum(RetroAmount),@ROUND_SALARY_UNIT) as RetroAmount
		,ROUND(SUM(RetroAmountNonTax),@ROUND_SALARY_UNIT) as RetroAmountNonTax
		,ROUND(SUM(MonthlyCustomAmount),@ROUND_SALARY_UNIT) as MonthlyCustomAmount
		,@PeriodID
		FROM #tblAllowance al
...
-- taxable allowance
 ...
 from #tblSalDetail sal,(
	...
	from (
		select EmployeeID,AllowanceID,ROUND(SUM(TaxableAmount),0) TaxableAmount,ROUND(SUM(RetroAmount),0) RetroAmount,ROUND(SUM(RetroAmountNonTax),0) RetroAmountNonTax
		from #tblAllowance group by EmployeeID,AllowanceID
	) al group by al.EmployeeID
 ) tmp where sal.EmployeeID = tmp.EmployeeID and sal.LatestSalEntry = 1
...
-- Nonetaxable allowance
 UPDATE #tblSalDetail set NoneTaxableAllowanceTotal = round(tmp.UntaxableAmount,@ROUND_SALARY_UNIT) from #tblSalDetail sal,(
	...
	from (
		select EmployeeID,AllowanceID,ROUND(SUM(UntaxableAmount),0) UntaxableAmount,ROUND(SUM(RetroAmountNonTax),0) RetroAmountNonTax from #tblAllowance group by EmployeeID,AllowanceID
	) al group by al.EmployeeID
 ) tmp where sal.EmployeeID = tmp.EmployeeID and sal.LatestSalEntry = 1

 UPDATE #tblSalDetail set TotalAllowanceForSalary = tmp.ReceiveAmount from
 #tblSalDetail sal,(
	 select al.EmployeeID,SUM(ISNULL(al.ReceiveAmount,0)) ReceiveAmount
	 from (
		select EmployeeID,AllowanceID,ROUND(SUM(ReceiveAmount),0) ReceiveAmount from #tblAllowance group by EmployeeID,AllowanceID
	 ) al
...
SELECT TOP 1 a.MinimumSal
 ...
 select * into #tmpBaseSalRegional from dbo.fn_CurrentBaseSalRegionalByDate(@ToDate)
 ...
 UPDATE #tblTradeUnion SET BaseSalaryRegional = bs.Salary
 from #tblTradeUnion a inner join #tblSalDetail b on a.EmployeeID = b.EmployeeID and b.LatestSalEntry = 1
 inner join #tmpBaseSalRegional bs on b.BaseSalRegionalID = bs.BaseSalRegionalID

 --Hiếu: tính tiền công đoàn
 update #tblTradeUnion
 set UnionFeeEmp = case when isnull(Emp_ByPercent,0) =1 then UNION_PERCENT_EMP * BaseSalaryRegional/100 else UNION_PACKAGE_EMP end
 * IsEmpPaid
 ...
 --Hiếu-JUNZ: chỉnh sửa cho NNN là không cần phải đóng công đoàn và công ty cũng không phải đóng
  update a set UnionFeeComp = 0
  from #tblTradeUnion a
  inner join #tblEmployeeIDList list on a.EmployeeID = list.EmployeeID
...
update #tblTradeUnion set UnionFeeComp = UNION_PACKAGE_COMP_MAX where UNION_PACKAGE_COMP_MAX > 0 and UnionFeeComp > UNION_PACKAGE_COMP_MAX
 ...
 select EmployeeID,ISNULL(UnionFeeEmp,0),ISNULL(UnionFeeComp,0)
	 ...
	 and Not exists(select 1 from #tblTradeUnion u where i.EmployeeID = u.EmployeeID)
...
from #tblSalDetail_ForTax t inner join(
	select EmployeeID, ROUND(SUM(ActualMonthlyBasic),0) ActualMonthlyBasic,SUM(UnpaidLeaveAmount) UnpaidLeaveAmount
...
group by a.EmployeeID) adj on sal.EmployeeID = adj.EmployeeID
 ...
 -- + insurance + gross từ nước ngoài
			...
			select EmployeeID,sum(TaxableAmount) as TotalGrossAllowanceAmount_Taxable
			from (select EmployeeID,AllowanceID,ROUND(SUM(TaxableAmount),0) TaxableAmount from #tblAllowance group by EmployeeID,AllowanceID) a
			inner join tblAllowanceSetting al on a.AllowanceID = al.AllowanceID
...
begin
	...
	FROM #TableVarTax txv, (SELECT * FROM tblTax tt WHERE tt.EffectDate = (SELECT MAX(EffectDate) FROM tblTax WHERE datediff(day,tt.EffectDate, @FromDate )>=0)) tx
	...
	UPDATE #TableVarTax SET
	TaxableIncome = ROUND(TaxableIncome,@ROUND_SALARY_UNIT), PITAmt = PITAmt
	...
	,PITAmt= ISNULL(PITAmt,0) +ISNULL(pit.totalPITRetro,0) from #TableVarTax tax
	inner join
	(
	...
	update #TableVarTax set TaxRetroImported = isnull(TaxRetroImported,0) + isnull(re.PIT_Retro_Amount,0)
		,PITAmt= ISNULL(PITAmt,0) +ISNULL( re.PIT_Retro_Amount,0)
...
update #TableVarTax set PITAmt = case when ISNULL(PITAmt,0) + a.PITAdjustment < 0 then 0 else ISNULL(PITAmt,0) + a.PITAdjustment end
from #TableVarTax p
inner join
(
...
--deduct advance and union
 ...
 UPDATE #tblSalDetail_ForTax
 set GrossTakeHome = ROUND(IncomeAfterPIT - ISNULL(OtherDeductionAfterPIT,0) - ISNULL(PITReturn,0) - ISNULL(AdvanceAmt,0) ,3)
...
update sal
	...
	from (select EmployeeID,AllowanceID,ROUND(SUM(ReceiveAmount),0) ReceiveAmount from #tblAllowance group by EmployeeID,AllowanceID) a
...
e.DepartmentID,e.SectionID,e.PositionID,sal.STD_WD,sal.BasicSalary,
...
begin
 ...
 SELECT EmployeeID,@Month,@Year,@PeriodID,ROUND(SUM(ActualMonthlyBasic),0)
