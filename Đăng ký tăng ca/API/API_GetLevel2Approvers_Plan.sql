...
GO
if object_id('[dbo].[API_GetLevel2Approvers_Plan]') is null
	EXEC ('CREATE PROCEDURE [dbo].[API_GetLevel2Approvers_Plan] as select 1')
GO

ALTER PROCEDURE [dbo].[API_GetLevel2Approvers_Plan]
    @LeaderEmpID varchar(50),
    @Level1EmpID varchar(50),
    @TypeRegister int = 1
as
begin
    set nocount on;
    declare @TargetOTTypeID int = case when @TypeRegister = 1 then 1 else 2 end;
    ...
    declare @App1 varchar(20), @App2 varchar(20);
    select top 1 @App1 = Approver_1, @App2 = Approver_2 from tblOT_ApprovalSetting ot
    where ot.DivisionID = @DivisionID and @TargetOTTypeID = OTTypeID;

    -- Nếu có setup cứng Cấp 2 thì trả về luôn
    if @App2 is not null
    begin
        select 2 as approver, @App2 as EmployeeID, @App2 + ' - ' + isnull((select FullName from tblEmployee where EmployeeID = @App2), '') as FullName, null as Level;
        return;
    end
    ...
    -- Xác định Level của Cấp 1
    declare @L1_Level int = 0;
    if @Level1EmpID <> 'SKIP' and @Level1EmpID <> ''
    begin
        select top 1 @L1_Level = isnull(pos.level, 0)
        from dbo.fn_vtblEmployeeList_Simple_ByDate(getdate(),'-1',null)   te left join tblPosition pos on te.PositionID = pos.PositionID
        where te.EmployeeID = @Level1EmpID;
    end

    -- Xuất danh sách Cấp 2 (Level lớn hơn Cấp 1)
    select distinct 2 as approver, e.EmployeeID, e.EmployeeID + ' - ' + isnull(te.FullName, '') as FullName, pos.level as Level
    from #EmpHierarchy e
    ...
    where e.level_id >= 5 and isnull(pos.level, 0) > @L1_Level
    order by pos.level;
